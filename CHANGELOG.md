...
- 对外产品名由「梁标」改为「梁向」；悬浮入口改为「今日梁向」，稳定技术标识 `dsh-liangbiao`、`liangbiao`、路由和数据前缀不变。
...
- 第一次 live 帧只记 earned 基线。Cmd+Shift+R 从 0 水合到真实炷数不再播「凝香 +1 炷」。梁子浮动与下一炷数字仍可照常出现。
...
- Client 半:向 `shell.overlay` 注册一个占位圆点(悬停/聚焦文案 `今日梁向`),无正式 UI。
