...
发 npm / GitHub Release 时，下面四条就是本号对外主文案。本号源码与社区后端已齐，npm / GitHub Release 仍等你点头。

### 梁祠香客

- 日梁、周梁、月梁详情都显示五行香客。
- 日梁是当日上过香的安装数；周/月按期内安装去重，不把同一香客按日加总。
- 社区库升到 schema v7，旧档案启动时按票表回填。

### 倾炉

- 长按满 **1.5 秒** 才倒空；未满松手仍撤销。短点仍是一炷。

### 面板

- 凝香环和香火/香客数字继续缓动；梁子跨相加一记短光，尊重减少动态。
- 窄窗或折叠侧栏：入口贴边，面板按剩余高度内部滚动。
- 此身香火在三界香火下常显；点一下可钉住说明。梁小号同样可点开本机提示。
- 入祠短音默认仍关，走现有音量档。六态图仍打在单文件 client.js 里，只做一次解码缓存——DSH 插件路由还是一个 JS。
