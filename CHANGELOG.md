...
- 新香客按设备指纹每个业务日送 10 炷；同机当日重装 / 换号不再重复领。对话攒香仍加在这份见面礼上。
- 首次欢迎页改疏：突出见面礼和倾炉，去掉密排说明。倒计时本来就已经去掉了。
- 安装包不再声明 `engines.node`。梁相不跟特定 DSH / Node 号绑定；DSH 宿主自己若要求 22.19+，仍由 DSH 决定。
- 仓库根增加 `screenshots.json`，awesome-dsh-plugin / dsh-market 换截图只需推本仓。
