...
- 本地 tarball 必须写成 `./dsh-liangxiang-<version>.tgz`，或 `file:$HOME/.../*.tgz`。漏掉 `./`、只写文件名、或写成 `dsh-liangxiang@0.8.3-beta` 时，pnpm 会去 npm 拉包并报 `ERR_PNPM_FETCH_404`（公开源还没有这个版本）。
- `update-plugin.sh` 改为始终传 `file:` 绝对路径；桌面目录可用 `install.sh`。
...
- 源码仓已从 `NiYa193/dsh-liang-meter` 迁移并重命名为 `liang-today/dsh-liangxiang`；`package.json` 的 repository / bugs 地址同步更新。
