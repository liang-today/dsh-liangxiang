...
`1.1.1`～`1.1.5` 为本次正式版的内部候选增量，没有单独对外发布。

- 梁相入口的默认停靠位置与 DSH alpha.5 设置按钮按真实几何对齐，在按钮上沿保留 8px 间隔；用户已拖动保存的位置不受影响。
- 对外更新说明同步收敛为四条用户收益，入口位置优化合并进既有交互文案。
- 同一份最终 tarball 同时通过 npm `latest` 的 DSH `0.1.1-rc.2` 与前瞻基线 `0.1.2-alpha.5` 真实 Profile 浏览器回归，无标签安装命令不要求用户切换预览通道。

## 1.1.5 — 未单独发布；汇入 1.1.6

- 发布候选兼容基线升级到 DSH `0.1.2-alpha.5`；上游已接管 v3/v4/v5 会话投影缓存兼容与坏记录备份跳过，本地启动不再预先搬走缓存。
- 新增正式发布总门禁，统一检查双仓隐私与历史、版本/部署一致性、模型当量、真实安装、功能、官网和逐项发布授权。

## 1.1.4 — 未单独发布；汇入 1.1.6

- 首次安装第二屏、升级自动提示和案牍“当前版本”收敛为同一个更新说明界面，内容与样式不再分叉。
- 首次欢迎与更新说明共用同一张 QQ 群二维码卡片，群号、邀请语、二维码和视觉样式统一维护。

## 1.1.3 — 未单独发布；汇入 1.1.6

- 欢迎页新增内嵌 QQ 群二维码，在克制的高亮卡片中直接扫码加入 `453683905`。
- 更新说明去掉多余的顶部引导句；梁相案牍的版本入口改为直接重看本次更新说明。
- 修复梁相案牍触发器与工具匣之间的悬停断层，鼠标可稳定移入菜单，离开整体范围后才收起。

## 1.1.2 — 未单独发布；汇入 1.1.6

- 长广播在梁小号原位单行循环，前置低调 × 可按广播 ID 在本浏览器关闭；下一条新广播仍会显示。
- 客户端更新说明压缩为上次正式版以来的四条用户收益，不再展示后台、安全、迁移和测试施工记录。
- 新增仓库级 `liangxiang-release-notes` Skill，固化对外文案密度与 npm / GitHub Release 发布前门禁。

## 1.1.1 — 未单独发布；汇入 1.1.6

本号曾是 Codex 接管后的校准与优化开发线，当时没有单独发布或部署；相关内容最终
随 `1.1.6` 统一验收和发布。

- 修复 Backend 批量 `count` 的幂等契约：SQLite schema v9 为进入 service 的
  accepted/rejected 业务处置保存 durable receipt，v7/v8 migration 整体原子化，并增加
  服务层、真实 HTTP 监听与本地重启回归。
- 兼容基线迁移到 DSH `0.1.2-alpha.4`，按钉住源码重新核对 Host、客户端和 Token 投影语义。
- 新增当前架构与事实索引，明确现行、历史和归档文档边界。
- 增加真实干净 Profile、浏览器无障碍和视觉冒烟基线，并补齐梁祠正反向焦点环。
- 六张梁子图改为可见像素等价的 lossless WebP，客户端 raw/gzip 体积约下降
  34%/41%，并增加构建预算和图片等价守卫。
- 修复亮暗主题可读性、reduced-motion 下残留动画，以及 Client dispose 未中止在途
  请求的问题。
- 干净 Profile 基线覆盖 DSH `0.1.2-alpha.4` 的 Node 22.23 / pnpm 11 安装政策、
  鉴权 WebUI 和 24 项桌面/窄屏/暗色浏览器回归。
- 官网与宣传素材按当前产品契约统一；后续视觉、性能和交互优化继续汇入本号。
- 每个版本首次打开会显示本号更新说明；新安装先完成使用指导，再进入更新说明。
- 恢复梁子在下一炷进度为 0 时的最慢待机浮动，入口与面板仍同步且尊重减少动态；六张原图没有重绘。
- 增加低打扰梁相广播：重要/紧急消息暂代梁小号，过期后自动恢复；首条消息公布 QQ 群 `453683905`。

## 1.0.7 — 2026-09-01

今日面板和梁祠这一轮手感。1.0.1～1.0.6 未单独上架的改动一并进本号。

npm `latest` 与 GitHub Release 发本号。社区后端 schema 未变，不必为这一号单独部署。

### 今日面板

- 第四区两列指标：上文字、下数字；数字在列内居中；标签 12px，与梁相案牍、进入梁祠同一行垂直居中；图标 26px，跨第四区整块上下居中。
- 此身香火只出现在三界香火的悬浮详情里，主界面不常显。
- 三界香火 / 五行香客悬停即出、移开即收，不再钉住。
- 梁相案牍点击打开，指针离开按键（含工具匣）即收。
- 进入梁祠离开按键不关；点梁祠外或 Esc 才退回主界面。
- 重启 WebUI 时，见面礼 10 炷只作账面回填，不再弹出「凝香 +10」。
- 上下放不开时移动图标让面板坐下，不再压缩面板高度。
- 声音默认一半；曾经调过喇叭的本机偏好仍保留。
- 跨相、入场、凝香动效更明显，仍尊重「减少动态」。

### 梁祠

- 日档五项按内容宽度一次排开，当日梁案让到右侧剩余空间，去掉横向滑动条。
- 日 / 周 / 月详情里，五行香客和数字在同一列。

### 一并到达（1.0.1 / 1.0.2）

- 新香客备 10 炷；同一台机器当天重装不再领一份。
- 梁祠日 / 周 / 月都记五行香客。
- 倾炉改为长按 1.5 秒。

## 1.0.6 — 2026-09-01

第四区按指标列重排：文字与按键同一行垂直居中，数字在列内居中。

发 npm / GitHub Release 时，下面就是本号对外主文案。本号源码已齐，npm / GitHub Release 仍等你点头。社区后端没有 schema 改动。

- 三界香火、五行香客改为 12px，与梁相案牍、进入梁祠同一字号，并在同一行垂直居中。
- 64、3 等数字落在各自列的水平中线，不再贴着文字左边。

## 1.0.5 — 2026-09-01

收回第四区最初的两行排版，梁祠日档不再横向滑动。

发 npm / GitHub Release 时，下面就是本号对外主文案。本号源码已齐，npm / GitHub Release 仍等你点头。社区后端没有 schema 改动。

- 三界香火、五行香客重新并排：上行文字对齐梁相案牍，下行数字对齐进入梁祠；不再上下叠成四行，也不再拉高两个按键。
- 梁祠日档五项按内容宽度一次排开，当日梁案让到右侧剩余空间，去掉横向滑动条。
...
- 三界香火、五行香客分两行：标签在上、数字在下，图标居中略放大；分别与梁相案牍、进入梁祠标齐。
- 统计用「下一炷」同款悬浮：移开即收，不再钉住。
- 梁相案牍点击打开，指针离开按键（含工具匣）即收。
- 梁祠离开按键不关；点梁祠外或 Esc 才退回主界面。
- 重启 WebUI 时，见面礼 10 炷只作账面回填，不再弹出「凝香 +10」。初始化仍可有梁子光效和声音。

## 1.0.3 — 2026-09-01

面板不再自己挤扁，香客数字回同一行，声音默认一半。

发 npm / GitHub Release 时，下面就是本号对外主文案。本号源码已齐，npm / GitHub Release 仍等你点头。社区后端没有 schema 改动，不必为这一号单独部署。

### 排版

- 梁祠日/周/月详情里，五行香客和数字在同一列，不再跟「1 案」缠到第二行。
- 今日面板的三界香火 / 五行香客改回「标签 + 数字」同一行。
- 此身香火只出现在点开三界香火后的详情里，主界面不再常显。
- 图标拖到上下放不开时，移动图标让整块面板坐下；不再压缩面板高度、把文字留在框外。

### 交互

- 三界香火与五行香客都是点击钉住详情，不会一移开就消失。
- 声音默认中档（插件音量一半，再叠系统音量）。曾经点过喇叭的本机偏好仍保留。
- 跨相、入场、凝香和梁位跳动都更明显，仍尊重「减少动态」。
