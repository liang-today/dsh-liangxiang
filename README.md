...
节点；数据与服务已经迁往香港，正式客户端统一连接
`https://api.liang.today`。域名切换完成前可用 `hk-api.liang.today` 旁路验收，
旧节点只保留限时透明转发，不再运行后端或持有写入权。尚未发布 npm 或
GitHub Release。迁移实录见 [`docs/142-hk-migration-report.md`](docs/142-hk-migration-report.md)。
