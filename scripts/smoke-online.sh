...
#
#   backend (node:sqlite) <- DSH host plugin <- /liangbiao/api/* <- curl
#
...
BACKEND_PORT="${LIANGBIAO_SMOKE_BACKEND_PORT:-4181}"
WEB_PORT="${LIANGBIAO_SMOKE_ONLINE_PORT:-3982}"
...
LIANGBIAO_BACKEND_PORT="$BACKEND_PORT" \
LIANGBIAO_BACKEND_DB="$DB_FILE" \
LIANGBIAO_SNAPSHOT_SECONDS=5 \
LIANGBIAO_TOKEN_PER_INCENSE=50000 \
LIANGBIAO_ALLOW_UNSIGNED=1 \
LIANGBIAO_COMMUNITY_KEY= \
...
if LIANGBIAO_AUTHORITY_MODE=VERIFIED_PRODUCTION LIANGBIAO_BACKEND_PORT=0 node lib/backend.js >/dev/null 2>&1; then
...
LIANGBIAO_BACKEND_URL="http://127.0.0.1:$BACKEND_PORT" \
LIANGBIAO_SNAPSHOT_SECONDS=5 \
  pnpm exec dsh --profile "$PROFILE" --port "$WEB_PORT" >"$WEB_LOG" 2>&1 &
WEB_PID=$!
wait_for "http://127.0.0.1:$WEB_PORT/liangbiao/api/state" "host api"

STATE="$(curl -fsS "http://127.0.0.1:$WEB_PORT/liangbiao/api/state")"
...
CLAIM="$(curl -fsS -X POST "http://127.0.0.1:$BACKEND_PORT/v1/token-claims" \
  ...
  -H "x-liangbiao-installation: $SMOKE_INSTALL" \
...
V1="$(curl -fsS -X POST "http://127.0.0.1:$BACKEND_PORT/v1/votes" \
  -H 'content-type: application/json' -H "x-liangbiao-installation: $SMOKE_INSTALL" -d "$VOTE_BODY")"
...
curl -fsS -X POST "http://127.0.0.1:$BACKEND_PORT/v1/token-claims" \
  -H 'content-type: application/json' -H "x-liangbiao-installation: inst-smoke-race-01" \
