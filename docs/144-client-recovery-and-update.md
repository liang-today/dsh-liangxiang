...
每个新的插件版本会在每个浏览器显示一次对应的「更新说明」：

- 首次安装：先完成原有使用指导与在线/离线选择，随后显示本版本更新说明；
- 老用户升级：直接显示本版本更新说明，不重复首装指导；
- 点击「知道了」后，以 `localStorage` 中的版本键记住已读。它只是浏览器外观偏好，
...
| 来源 | 升级命令 |
|---|---|
| 插件市场 | 设置 → 插件市场 → 检查更新 / 更新。比对 npm `latest`，不必先 remove |
| npm | 重复安装命令 `add dsh-liangxiang`（解析 `latest`）。没有全局 `dsh` 时把开头换成 `npx --yes @deepseek-ai/dsh`。支持版本浮动的 Host（v1.0.0 起）会把精确号 / tarball / 残留 `beta` 改成 `latest`，不必先 remove |
...
bash scripts/update-plugin.sh ./dsh-liangxiang-1.0.7.tgz --profile <profile名>
```
