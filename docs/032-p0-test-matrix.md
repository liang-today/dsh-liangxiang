...
四区顺序 case/core/vote/social、仅两个投票按钮（夯：升梁！/拉：降梁！，等宽标齐）、dialog 标题今日梁案、梁位与梁圣同快照、环居中且两翼 overlay、零票 `--`+待开梁、remaining 0 双按钮 disabled + 可访问 reason、六态头像两两视觉不同、香火 12,846 / 香客 2,841（`🪔`/`🙏` 图标 + 15px 文案）、case 区居中且无可见「本地演示」徽标（软信任标注在 `data-liangbiao-authority` + SR 摘要）。→ `client-panel.spec.tsx`、`badge.spec.tsx`
