...
共享的是同一台梁标后端上的今日梁案；信任模型公开写成软信任，不写成民调、不写成全网公投。
...
4. **没有运营面** — 改今日梁案曾要改环境变量并重启；现已有 `POST /v1/admin/cases`（社区口令）
...
| 运营改案 | `POST /v1/admin/cases` + 社区口令：归档当前案并开新案（见 [`076`](076-backend-api-v1.md)、[`121`](121-vps-deploy.md)） |
