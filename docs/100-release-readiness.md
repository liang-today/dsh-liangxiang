# 100 — Release Readiness（v1.0.7 梁相）

结论：**社区正式版已按 1.0.7 发布**；不得宣传为实名、一人一票、
...
对外主文案见 [`CHANGELOG.md`](../CHANGELOG.md) 的 `1.0.7`：今日面板第四区与梁祠排版、悬浮即走、见面礼不再弹窗，以及 1.0.1 / 1.0.2 未上架的见面礼与香客。GitHub Release 发本号；npm `latest` 由维护者发包。后端 schema 未变。1.0.3～1.0.6 不单独上架，随本号一次发出。

## 1.0.2 点更新

对外主文案见 [`CHANGELOG.md`](../CHANGELOG.md) 的 `1.0.2`：梁祠日/周/月补五行香客，倾炉改为 1.5 秒，窄窗停靠与本机痕迹可发现。

## 本版变更

- 审查修复：启动观察按序重放 catch-up 基线；幂等重放计数取当前账面；`pointercancel` 不再误投。
...
- 服务器统一命令可查询、发行、作废入梁券；每个业务日 0 点把剩余认领补到 1,000，不整批替换。
...
- 正式版安装/升级同一条命令：`dsh plugin --profile web add dsh-liangxiang`（`latest`）。Host 启动时把精确号、tarball、残留 `beta` 改成 `latest`，并排除 pnpm 11 的 24 小时发布冷静期；启动不跑 `pnpm add`。
...
`0.8.0` 仍留在 registry 且不可覆盖。1.0.0 高于既有号，作为首个正式号占据
`latest`；`beta` 标签同时指到 1.0.0，只为兼容旧书签。安装文档写无标签包名。
不得把 registry 默认标签解释为已核验的稳定公投。
...
4. 桌面 `liangxiang` 目录中的 tarball 已按内容、密钥和 SHA-256 审计通过
   （`dsh-liangxiang-1.0.0.tgz` /
   SHA-256 `0438fb576301a9621871fc9c58ebd03e5a79659fab64cf8ffb96f176fc09f56c`）。
