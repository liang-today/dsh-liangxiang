# 102 — Known Limitations（更新至 v0.5.0）
...
4. **正式 TLS 尚未切换**。代码默认监听 `127.0.0.1`，社区测试节点目前为临时 IP/HTTP，并使用 Ed25519 安装签名与共享闭测口令；香港迁移和 `api.liang.today` TLS 完成前不按正式公开服务宣传。
...
12. v0.5.0 沿用 SQLite v4 与 `/v1/history`；梁祠失败继续保留最后一次正确档案并显示「档案未更新」，不阻断今日投票。
