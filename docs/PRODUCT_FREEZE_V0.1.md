...
4. **社会化** — `三界香火` 与 `五行香客` 并排、上文字下数字且数字列内居中，上行与 **梁相案牍** 垂直居中、下行与 **进入梁祠** 垂直居中。悬停三界香火可在详情里看到本机 **此身香火**（今日/累计与占梁），移开即收，主界面不常显；不是第五区、不是天庭账。日常同步完全自动；核香只用于显示异常后的服务器账本修复；模式只能由用户明确切换。
