...
`<60% 梁工；[60,70) 梁总；[70,80) 梁神；[80,90) 梁圣；>=90% 梁祖`。阈值策略可配置且必须通过校验：四个边界、(0,1) 内、严格递增（无重叠无缺口）。
测试 `domain-liangzi.spec.ts`（59.999/60/69.999/70/79.999/80/89.999/90/100%）。
...
一票使 up_ratio 从 79.x% 跨到 80%（79/20 → 80/20），状态梁神→梁圣；同时投票者个人 remaining -1、fill 不变。状态变化原因是比例，不是个人库存。
测试 `domain-independence.spec.ts`、`client-store.spec.ts`。
