...
> **R2 语义修订(2026-08-16)**:本文写于旧产品语义时期。DSH 技术事实(触点、事件、投影、通道、存储)仍有效;但涉及梁向业务语义处已按 [`PRODUCT_FREEZE_V0.1.md`](PRODUCT_FREEZE_V0.1.md) 就地修订——旧「梁签 / tokensPerBallot / cacheRead×0.1 / per-request cap / 目标模型口径」模型全部废弃,见 [`SEMANTIC_CORRECTION_R2.md`](SEMANTIC_CORRECTION_R2.md)。

## 1. 产品回顾(冻结契约的架构映射)

梁向(`dsh-liangbiao`)把 DSH 的 Input+Output Token 折算为个人香火(默认 50,000 Token = 1 炷),用户对当天唯一二元梁案投「夯/拉」(1 票 = 1 炷,夯/拉共用一个香火池);全网夯率唯一决定中央梁子的待开梁/五态;个人剩余香火 + 距下一炷 Token 进度构成梁气。悬停文案恒为 `今日梁向`。v0.1 仅 DSH WebUI 插件;Host 拥有权威本地状态,浏览器只是视图与命令面;离线必须可渲染。
...
flowchart LR
  ...
  subgraph browser [DSH WebUI]
    ...
    clientCore["client/: 梁向徽章 + 详情面板<br/>SSE 订阅 → HostObservable"]
    clientCore --> compatC
  end
...
- `identity`:梁向自铸匿名安装 id(UUID v4,仿 `dsh-anonymous-user-id` 的存储姿态但**不复用**其 id,避免与 telemetry/provider 关联;见 `docs/004` 隐私项)。
...
- 主题与可达性:只用 `var(--dsw-*)` 语义 token;暗色跟随 `body[data-ds-dark-theme]`;动画尊重 `@media (prefers-reduced-motion: reduce)`;悬停/聚焦均出 `今日梁向` tooltip;键盘可达(条目为 button,Enter 展开详情面板)。香火环四阶段(0–69 冷调、70–89 暖调、90–99 朱红、100 一次完成动画后稳态)在 CSS 变量上实现,资产为原创 CSS/SVG。
...
- **前提**:组合挂载了 token-meter + session-projection(web 组合默认;两者缺任一时梁向显示"记账不可用"但 UI 仍渲染)。
...
- 会话日志出现未知必读事件导致某能力缺席时,梁向显示"记账暂停"而非猜测。
