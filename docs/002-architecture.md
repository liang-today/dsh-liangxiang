...
> **R2 语义修订(2026-08-16)**:本文写于旧产品语义时期。DSH 技术事实(触点、事件、投影、通道、存储)仍有效;但涉及梁相业务语义处已按 [`PRODUCT_FREEZE_V0.1.md`](PRODUCT_FREEZE_V0.1.md) 就地修订——旧「梁签 / tokensPerBallot / cacheRead×0.1 / per-request cap / 目标模型口径」模型全部废弃,见 [`SEMANTIC_CORRECTION_R2.md`](SEMANTIC_CORRECTION_R2.md)。

## 1. 产品回顾(冻结契约的架构映射)

梁相(`dsh-liangxiang`)把 DSH 的 Input+Output Token 折算为个人香火(默认 50,000 Token = 1 炷),用户对当天唯一二元梁案投「夯/拉」(1 票 = 1 炷,夯/拉共用一个香火池);全网夯率唯一决定中央梁子的待开梁/五态;个人剩余香火 + 距下一炷 Token 进度构成梁气。悬停文案恒为 `今日梁相`。v0.1 仅 DSH WebUI 插件;Host 拥有权威本地状态,浏览器只是视图与命令面;离线必须可渲染。

## 2. 分层与代码布局(首选)

单 npm 包 `dsh-liangxiang`,双面(Host + Web Client),目录即分层:
...
> **Phase 3 更新(2026-08-16)**:`backend/` 不再是占位接口,而是可运行的 localhost 权威服务(`/v1/*` + SQLite),Host 以 `LIANGXIANG_BACKEND_URL` 切换 `LOCAL_FAKE_DEV` / `DEV_STAGING_ONLY`。全链路见 [`070`](070-backend-architecture.md),schema 见 [`071`](071-database-schema.md),API 见 [`076`](076-backend-api-v1.md),信任边界见 [`075`](075-backend-decision.md)。
...
flowchart LR
  ...
  subgraph browser [DSH WebUI]
    ...
    clientCore["client/: 梁相徽章 + 详情面板<br/>SSE 订阅 → HostObservable"]
    clientCore --> compatC
  end
  shared["shared/: wire 契约 + 校验"]
  hostCore -- "GET /liangxiang/api/state<br/>GET /liangxiang/api/events (SSE)<br/>POST /liangxiang/api/vote" --> clientCore
  hostCore -.-> shared
  clientCore -.-> shared
...
- `cordis.patch.yml` 只插入一行 host 插件(`- insert: [{ id: liangxiang, name: dsh-liangxiang }]`);Client 半由 `dsh.client` 清单扫描自动接入,无需额外行。
...
- 安装:`dsh plugin --profile <name> add dsh-liangxiang`;因声明了 `dsh.bundle`,CLI 自动把包名追加进 `dsh.profile.bundles`。
...
| R6 | **(R2 修订)** `earned = floor(effectiveTokensToday / tokenPerIncense)`,`tokenPerIncense` 默认 50,000、取自梁案配置;余数即香火环进度(不"铸签") | AGENTS.md §5 |
...
`defineDomain({ name: 'liangxiang', version: 1, tables: … })`,经 `ctx.storageDomain.open()` 落 `$DSH_HOME/storages`(001-Q13)。逻辑表(R2 修订后):
...
- `identity`:梁相自铸匿名安装 id(UUID v4,仿 `dsh-anonymous-user-id` 的存储姿态但**不复用**其 id,避免与 telemetry/provider 关联;见 `docs/004` 隐私项)。
- `remote_snapshot`:缓存的全局快照(香火=总接受票、香客=独立参与用户;离线渲染用)。

用户偏好(非账本)走 `ctx.settings.register('liangxiang', schema)`:徽章显隐、贴边偏移等。

### 4.3 Host↔Client 通道(自建 HTTP + SSE)

`ctx.webServer.register`(001-Q6)三个端点,路径前缀 `/liangxiang/api/`:

- `GET /liangxiang/api/state`:全量快照(梁案、全局快照[比例+梁子状态同版本]、个人梁气、单调 `revision`)。
- `GET /liangxiang/api/events`:SSE 增量推送(快照变化即推新 `revision` 帧;重连由客户端先 GET state 再挂 SSE,revision 落后帧丢弃)。
- `POST /liangxiang/api/vote`:`{ caseId, voteType: "up"|"down", requestId }`;同键重放返回首次结果;一票消耗一炷香。

wire 类型与校验在 `shared/`;Host 对入站 body 逐字段校验(边界校验原则)。不触碰 ApiProxy/HostFrame 封闭词表;不用 `@Remote`(树外不可用,001-Q6)。

## 5. Client 端设计

- 注册:`ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'liangxiang', order: 100, inject: … }, Badge))`(001-Q3/Q4/Q5)。贴右由条目自身 CSS 定位(overlay 层 `inset:0` click-through,条目 `pointer-events:auto`),避开 composer/导航,不遮挡 `details` 右栏常用区。
...
- 主题与可达性:只用 `var(--dsw-*)` 语义 token;暗色跟随 `body[data-ds-dark-theme]`;动画尊重 `@media (prefers-reduced-motion: reduce)`;悬停/聚焦均出 `今日梁相` tooltip;键盘可达(条目为 button,Enter 展开详情面板)。香火环四阶段(0–69 冷调、70–89 暖调、90–99 朱红、100 一次完成动画后稳态)在 CSS 变量上实现,资产为原创 CSS/SVG。
...
- **前提**:组合挂载了 token-meter + session-projection(web 组合默认;两者缺任一时梁相显示"记账不可用"但 UI 仍渲染)。
...
- 会话日志出现未知必读事件导致某能力缺席时,梁相显示"记账暂停"而非猜测。
