...
基线 `47f94385`。每项：结论 + 源码证据 + 梁向落点。全部经 `src/compat/dsh/` 隔离。
...
- 梁向落点：单条 prefix 路由 `/liangbiao/api`（state / events(SSE) / vote），`compat/dsh/web-routes.ts`。
...
- 梁向落点：`compat/dsh/usage-observer.ts`。
...
- 梁向落点：domain 名 `liangbiao` v1，表 `watermarks / daily_usage / ledger / votes / aggregates`（`compat/dsh/storage.ts`）。梁向不引入 zod 依赖——spec 的 `valueSchema` 传入自写窄校验对象（运行时仅要求 `.parse(raw)`，spec.ts:29 + index.ts:121 验证）。
...
- 梁向落点：host `apply` 顶层零 inject;三个嵌套 inject（`webServer` / `storageDomain` / `sessionProjections`+`sessions`），任一缺席时对应能力降级（记账不可用 / 内存态），UI 仍渲染（冻结需求 9）。
...
- toast/dialog：`ui-primitives` 为树内 body-portal 组件，未对树外发布稳定入口——梁向面板内自绘反馈行，不依赖。

## Business date

DSH 无时区/业务日期设施（040-E）。梁向 Host 自持 `BusinessDateProvider`（Intl.DateTimeFormat + 显式 dev timezone 配置，默认 `Asia/Shanghai`，`LIANGBIAO_BUSINESS_TZ` 覆盖）;在线后以 Backend 为准。
