...
- 仅允许 `liangops` 使用公钥 SSH；root、密码、键盘交互认证、X11、agent、
  TCP/Unix socket 转发与 tunnel 均关闭。
- 云防火墙之外再启用 firewalld：HTTP/HTTPS 对公网开放。应运营者临时外出
  登录需要，SSH 来源暂时对公网开放；认证仍严格限定 `liangops + publickey`，
  root、密码认证和转发均保持禁用。
...
- 当前桌面分发包为 `dsh-liangxiang-0.8.0.tgz`，SHA-256 为
  ...
  不沿用迁移期旧包摘要。
- 当前本地功能候选为 `dsh-liangxiang@0.8.4-beta`；迁移后的 0.8.1-beta.0
  MacBook 复验属于历史证据，0.8.4-beta 仍须在最终打包后补真实 profile 验收。
